...
**Phase 1A covers is implemented but not physically Kindle-verified.**
...
5. Create `ui/picker.lua`, then add a “Browse and download” menu item offering
   Inbox, Later, and Shortlist. Fetch metadata and render title-first rows with
   safe fallbacks; show site/author and time only if supported by the verified UI.
6. Use the verified KOReader multi-selection pattern, show selected count, ask
...
| File | Responsibility |
| --- | --- |
| `readwisereader.koplugin/api/reader.lua` | Reader list/detail/update requests. |
| `readwisereader.koplugin/library/covers.lua` | Temporary binary download and native cover installation. |
| `readwisereader.koplugin/ui/picker.lua` | Location chooser, selection state, and feedback. |
